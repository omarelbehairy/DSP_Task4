...
with first_img_container_left:
    left_column, right_column = st.columns(2)
    with left_column:
        uploaded_file1 = st.file_uploader("Choose an image1 file",type=['jpg','jpeg','png'])
...
with second_img_container_left:
    left_column, right_column = st.columns(2)
    with left_column:
        uploaded_file2 = st.file_uploader("Choose an image2 file",type=['jpg','jpeg','png'])
        logger.info('Uploading image 2...')

    with right_column:
        option2 = st.selectbox("choose the desired image2 transformtion",('FT Magnitude','FT Phase','FT Real','FT Imaginary'))
...
if uploaded_file1 is not None and uploaded_file2 is not None:
    ...
    class TotalGui:
        def __init__(self):
            ...
            self.magnitudeSpec1,self.phaseSpec1,self.realSpec1,self.imagSpec1 = ap.modes_for_the_image(self.image1)
            self.magnitudeSpec2,self.phaseSpec2,self.realSpec2,self.imagSpec2 = ap.modes_for_the_image(self.image2)
        ...
        def drawOP(self,Total_img):
            if 'Total_img'  not in st.session_state:
                ...
                with result_img_container_right:
                    with left_column:
                        if((comp1!=comp2)and(comp1=='img2')):
                            Total_img = ap.mix_photos(self.image2,self.image1,mixrate1,mixrate2,mix_comp1,mix_comp2)
                            self.drawOP(Total_img)  
                        elif(comp1==comp2=='img1'):
                            Total_img = ap.mix_photos(self.image1,self.image1,mixrate1,mixrate2,mix_comp1,mix_comp2)
                            self.drawOP(Total_img)
                        elif(comp1==comp2=='img2'):
                            Total_img = ap.mix_photos(self.image2,self.image2,mixrate1,mixrate2,mix_comp1,mix_comp2)
                            self.drawOP(Total_img)
                        else:
                            Total_img = ap.mix_photos(self.image1,self.image2,mixrate1,mixrate2,mix_comp1,mix_comp2)
                            self.drawOP(Total_img)
