...
def produce_mix(image1,image2):
    ...
    uniform_magnitude1 = np.ones(magnitude1)
    uniform_phase1 = np.zeros(phase1)
    
    uniform_magnitude2 = np.ones(magnitude2)
    uniform_phase2 = np.zeros(phase2)
...
def mix_photos(image1, image2, mixing_ratio1,mixing_ratio2,comp1,comp2):
    ...
    if((comp1 == 'real' or comp1 == 'imaginary') and (comp2 == 'real' or comp2 == 'imaginary')):
        ...
        mixed_image = np.fft.ifft2(mixed_fft).real.astype(np.uint8)

    elif((comp1 == 'Mag' ) and ( comp2 == 'phase')):
        ...
        mag_mix = uniform_magnitude1*(mixing_ratio1/100)+magnitude2*(1-(mixing_ratio1/100))
